...
export default function SeriesCapacitors() {
  const [capacitors, setCapacitors] = useState([]);
  const [capacitorsData, setCapacitorsData] = useState(capacitors);
  const [filteredCapacitors, setFilteredCapacitors] = useState([]); // New state for filtered capacitors
  ...
  const downloadTableAsExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Series Capacitor List");
    XLSX.writeFile(wb, "series_capacitor_list.xlsx");
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      setFilteredCapacitors(data); // Update filteredCapacitors when new data is uploaded
    };
  ...
  useEffect(() => {
    const fetchCapacitors = async () => {
      ...
      try {
        ...
        if (response.ok) {
          setCapacitors(data.capacitors || []);
          setCapacitorsData(data.capacitors || []);
          setFilteredCapacitors(data.capacitors || []); // Initialize filteredCapacitors
        } else {
          setError(data.error || "Failed to fetch Series Capacitors");
        }
      } catch (error) {
        console.error("Error fetching Series Capacitors:", error);
        setError("Failed to fetch Series Capacitors due to a network error");
      } finally {
        setLoading(false);
      }
    };
  ...
  // Filter capacitors based on search query (by location)
  useEffect(() => {
    let filtered = capacitorsData;

    if (searchQuery) {
      filtered = filtered.filter((capacitor) =>
        (capacitor.location || "").toLowerCase().includes(searchQuery.toLowerCase())
      );
    }

    setFilteredCapacitors(filtered);
  }, [searchQuery, capacitorsData]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        const updatedCapacitors = capacitors.filter((capacitor) => capacitor._id !== id);
        setCapacitors(updatedCapacitors);
        setCapacitorsData(updatedCapacitors);
        setFilteredCapacitors(updatedCapacitors); // Update filteredCapacitors after deletion
      } else {
        const data = await response.json();
        setError(data.error || "Failed to delete Series Capacitor");
      }
    } catch (error) {
      console.error("Error deleting Series Capacitor:", error);
      setError("Failed to delete Series Capacitor due to a network error");
    }
  };
  ...
  return (
    <div className="font-bold text-4xl">
      ...
      <div className="flex items-start space-x-4 mb-6">
        ...
        <Link href="/series-capacitor/create">
      ...
      <div className="container mx-auto my-6 px-4 border border-gray-300 shadow-xl rounded-lg">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white mb-5 shadow-md rounded-sm table-auto border border-[#F1F5F9]">
            ...
            <tbody>
              {filteredCapacitors.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-4 font-normal text-sm">
                    No Series Capacitors available
                  </td>
                </tr>
              ) : (
                filteredCapacitors.map((capacitor, index) => (
