...
export default function Loads() {
  ...
  useEffect(() => {
    const fetchIbrs = async () => {
      try {
        ...
        if (response.ok) {
          setIbrs(data.ibrs || []); // Ensure the loades array is being set correctly
          setIbrsData(data.ibrs || []); // Ensure loadesData is also updated
        } else {
          console.error("Failed to fetch Ibrs:", data.error); // Log error if response is not ok
        }
      } catch (error) {
        console.error("Error fetching Ibrs:", error); // Log error in case of network issues
      }
    };
  ...
  const handleDelete = async (id) => {
    ...
    if (response.ok) {
      ...
      setIbrs(ibrs.filter((ibrs) => ibrs._id !== id));
      setIbrsData(ibrs.filter((ibrs) => ibrs._id !== id)); // Update loadesData
    } else {
