...
import ShuntCapacitor from '@/models/shuntCapacitor';   // Load mongoose model
...
export async function POST(req) {
  try {
    ...
    return NextResponse.json(
      { message: 'Load created successfully', shunt: newShuntCapacitor },
      { status: 201 }
    );
  } catch (error) {
...
export async function GET(req) {
  try {
    ...
    const shunts = await ShuntCapacitor.find();
